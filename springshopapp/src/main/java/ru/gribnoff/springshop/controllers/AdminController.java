...
public class AdminController {
	...
	public String removeReview(@PathVariable UUID id) throws NotFoundException {
		reviewsToApprove.removeIf(review -> review.getId().equals(id));
		Review review = reviewService.getReviewById(id).orElseThrow(() -> new NotFoundException("Review not found"));
	...
	public String approveReview(@PathVariable UUID id) throws NotFoundException {
		reviewsToApprove.removeIf(review -> review.getId().equals(id));
        Review review = reviewService.getReviewById(id).orElseThrow(() -> new NotFoundException("Review not found"));
